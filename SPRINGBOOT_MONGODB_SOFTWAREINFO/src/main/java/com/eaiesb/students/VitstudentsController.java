...
public class VitstudentsController {
	...
	@GetMapping("/studentS/{campid}")
	public List<Vitstudents> getByCampidand1Depid(@PathVariable String campid){
		return service.getByCampidDepid(campid);
	}
	
	@GetMapping("/studentss/{depid}/{campid}")
    public List<Vitstudents> getStudentsByDepartmentAndCampus(
            @PathVariable String depid, @PathVariable String campid) {
        return service.getStudentsByDepartmentAndCampus(depid, campid);
    }
